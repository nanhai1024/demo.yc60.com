	...
	class LinkAction extends Action {
		...
		private function state() {
			if (isset($_GET['id'])) {
				...
				if (!$this->_model->getOneLink()) Tool::alertBack('警告：不存在此链接！');
				if ($_GET['type'] == 'ok') {
					$this->_model->setStateOK() ? Tool::alertLocation(null,PREV_URL) : Tool::alertBack('警告：审核链接失败！');
		...
		private function add() {
			if (isset($_POST['send'])) {
				...
				$this->_model->addLink() ? Tool::alertLocation('恭喜，新增友情链接成功！','?action=show') : Tool::alertBack('很遗憾，新增友情链接失败，请重试！');
			}
		...
		private function delete() {
			if (isset($_GET['id'])) {
				$this->_model->id = $_GET['id'];
				$this->_model->deleteLink() ? Tool::alertLocation('恭喜你，删除链接成功！', PREV_URL) : Tool::alertBack('很遗憾，删除链接失败！');
			} else {
				Tool::alertBack('非法操作！');
			}
		}
