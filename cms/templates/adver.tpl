...
<table cellspacing="0">
	...
	<tr>
		...
		<td>{@value->type}</td>
		<td>{@value->state}</td>
		<td><a href="adver.php?action=update&id={@value->id}">查看并修改</a> | <a href="adver.php?action=delete&id={@value->id}" onclick="return confirm('你真的要删除这个广告吗？') ? true : false">删除</a></td>
	</tr>
	{/foreach}
